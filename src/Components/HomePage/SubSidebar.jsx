    ...
    return (
        <div className='w-[5.5rem] h-full flex flex-col items-center justify-between py-7 bg-[#F0F4FA]'>
            <div className='w-[3.5rem] flex flex-col items-center space-y-7 '>
                ...
                <div className={`${(show === "Chats" || show === "") && 'bg-theme text-white transition-colors'} p-2 rounded-lg`} onClick={() => setShow("Chats")}>
                    <BiMessageSquareDetail className='' size={20} />
                </div>

                <div className={`${show === "People" && 'bg-theme text-white transition-colors'} p-2 rounded-lg`} onClick={() => setShow("People")}>
                    <MdOutlineGroupAdd className='' size={20} />
                </div>
                
                <div className={`${show === "Group" && 'bg-theme text-white transition-colors'} p-2 rounded-lg`} onClick={() => setShow("Group")}>
                    <MdOutlineGroups className='' size={20} />
                </div>

                <div className={`${show === "Calls" && 'bg-theme text-white transition-colors'} p-2 rounded-lg`} onClick={() => setShow("Calls")}>
                    <IoCallOutline className='' size={20} />
                </div>

                <div className='w-full h-[1px] bg-neutral-500'></div>

                <div className={`${show === "Setting" && 'bg-theme text-white transition-colors'} p-2 rounded-lg`} onClick={() => setShow("Setting")}>
                    <AiOutlineSetting className='' size={20} />
                </div>
            </div>
            <div className='w-[3.5rem] flex flex-col items-center space-y-7 '>
                <div className='w-[2.2rem] bg-theme rounded-full p-[3px]'>
